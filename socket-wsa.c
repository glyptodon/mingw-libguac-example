...
/**
 * Data associated with an open socket which writes to a WSA socket.
 */
typedef struct guac_socket_wsa_data {

    /**
     * The associated socket handle.
     */
    SOCKET sock;
...
/**
 * Writes the entire contents of the given buffer to theWSA socket 
 * associated with the given socket, retrying as necessary until the whole
 * buffer is written, and aborting if an error occurs.
 *
 * @param socket
 *     The guac_socket associated with the WSA socket to which the given
...
/**
 * Attempts to read from the underlying WSA socket of the given
